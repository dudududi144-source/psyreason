...
export class Engine {
  ...
  vKick(t: number, vel = 1) {
    ...
    if (this.params.bass.sidechain > 0) {
      ...
      pd.setValueAtTime(1 - 0.14 * this.params.bass.sidechain, t);
      pd.setTargetAtTime(1, t + 0.03, 0.13);
    }
  }
  ...
  private schedule(g: number, t: number) {
    ...
    const on = (id: TrackId) => {
      ...
      if (roleNow === 'intro') {
        ...
        if (id === 'bass') return frac >= 0.25;
        if (id === 'kick') return frac >= 0.5;
        if (id === 'hats' || id === 'open') return frac >= 0.6;
        if (id === 'lead') return frac >= 0.75;
      }
